...
function title(job: JobRecord) { return job.station?.generated_title || job.manifest?.title || 'Untitled meeting' }
  ...
  return <div>
    ...
    {result && <>
      ...
      <div id="mi-overview" role="tabpanel" aria-labelledby="mi-tab-overview" hidden={tab !== 'overview'} className="mi-report">
        ...
        <p className="mi-report-footnote">Generated locally from the full discussion. Audio warnings mark potentially misheard passages; unsupported claims remain in Needs review.</p>
      </div>
...
function EvidenceView({ item, onJump }: { item: SourcedItem; onJump: (item: SourcedItem) => void }) {
  ...
  return <div className={`mi-evidence ${item.source_check === 'passed' && !reviewRequired ? 'is-checked' : 'needs-review'}`}><div><i aria-hidden="true" /><span>{item.source_check === 'passed' ? 'Matches transcript' : 'Source needs review'}</span>{item.audio_warning && <span className="mi-review-label">Check quoted audio</span>}{reviewRequired && <span className="mi-review-label">{item.review_status === 'rejected' ? 'Rejected in review' : 'Needs review'}</span>}{item.review_status === 'human_confirmed' && <span className="mi-review-label is-confirmed">Human confirmed</span>}{hasSource && <button onClick={() => onJump(item)}>{item.evidence.start != null ? time(item.evidence.start) : 'View transcript'}</button>}</div>{item.evidence.quote && <blockquote dir="auto">“{item.evidence.quote}”</blockquote>}</div>
}
