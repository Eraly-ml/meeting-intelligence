...
export function MeetingBrowserPanel({ open: expanded, onOpenChange, config, connected, jobs, onCreated, diarizationAvailable }: {
  ...
  if (!expanded && !activeId && !pendingRecordings.length && !error) return null
    ...
    {expanded && <div id="mi-browser-controls" className="mi-browser-controls">
      {!connected ? <p className="mi-muted">Pair this station to open its meeting browser.</p> : <>
        ...
        <div className="mi-browser-record-options mi-form">
          ...
          <div className="mi-browser-record-actions"><Button disabled={!available || !!busy || !!activeId || status?.state !== 'opened'} onClick={() => void start()}>{busy === 'start' && <Loader2 className="animate-spin" />}Start recording</Button><span>The meeting platform uses the internet. Transcription and report generation run locally after recording stops.</span></div>
        </div>
      </>}
    </div>}
