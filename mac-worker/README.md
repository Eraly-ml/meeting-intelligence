...
Configure all paths in `.env`. Generate a long random `MI_API_TOKEN`; the service refuses an empty or short token. A LAN bind requires `MI_TLS_CERT_FILE`, `MI_TLS_KEY_FILE` and `MI_TLS_CLIENT_CA_FILE`: mutual TLS is mandatory and the worker requires a trusted client certificate. The station pins the private CA and verifies the Mac's IP certificate. Provision identities with `scripts/create-worker-pki.py`; see [security and recovery](../docs/SECURITY.md). Keep the Mac awake for processing.

The default ASR profile is **whisper-cpp** for all languages. `MI_WHISPER_BINARY` accepts an absolute local executable path, `MI_WHISPER_MODEL` a downloaded multilingual GGML model, and `MI_FFMPEG_BINARY` any compatible local ffmpeg executable. Full large-v3 is installed as the multilingual default. `MI_WHISPER_MODEL_EN` selects turbo q5 for explicitly English input based on the available fixture. Provision weights beforehand and verify the [official model manifest](https://github.com/ggml-org/whisper.cpp/blob/master/models/README.md). Other adapters remain optional and need installed packages and local model directories. See the [accuracy audit](../docs/ACCURACY.md) for measured results and missing human/multilingual evidence.
...
PDF exports include the summary, decisions, topics and key points, open questions, risks, action table, evidence references and timestamped transcript. An empty protocol is explicitly marked for review and starts the transcript on the first page; headings alone are never presented as a completed report. Summary references point to their source items.

Provision `MI_WHISPER_VAD_MODEL` with the local Silero 6.2.0 model from the official [whisper-vad repository](https://huggingface.co/ggml-org/whisper-vad). A configured but missing VAD file fails readiness checks. Whisper uses beam 5 and `-mc 0` to limit recognition loops; repeated output and low token probabilities remain review signals, not proofs of correctness.
